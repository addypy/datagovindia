...
class git_assets:
    ...
    def __init__(self):
        ...
        # 
        print('.... Step (2/2) - Loading latest API reference data. This may take a few seconds.............. \r',end='')
        ...
        print('.... Step (2/2) - Loading latest API meta-data. This may take a few seconds..........\r',end='')
        ...
        self.idx_creationtime_map   = [{k.get("resourceid"):k.get("date_created",np.nan)} for k in self.idx_map]
        self.idx_updationtime_map   = [{k.get("resourceid"):k.get("date_updated",np.nan)} for k in self.idx_map]    
    ...
    def compile_all_information(self,rsrc_id,api_key):
        ...
        if rsrc_id in self.resource_ids:            
            ...
            org_type    = np.ravel([list(item.values()) for item in self.idx_orgtype_map if list(item.keys())[0]==rsrc_id][0])[0]
            sector      = np.ravel([list(item.values()) for item in self.idx_sector_map if list(item.keys())[0]==rsrc_id][0])[0]
            source      = np.ravel([list(item.values()) for item in self.idx_source_map if list(item.keys())[0]==rsrc_id][0])[0]
            fields      = list(np.ravel([list(item.values()) for item in self.idx_field_map if list(item.keys())[0]==rsrc_id][0]))
            ApiInformation = {"ResourceID":rsrc_id,
                             "Title"    :title,
                             "Description":desc,
